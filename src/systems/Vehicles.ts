...
const PED_REACH_MARGIN = 0.5; // added to car.profile.radius for pedestrian contact check
...
const POLICE_FEELER_BASE = 7; // feeler look-ahead added to car.profile.radius
...
export class Vehicles {
  ...
  pedestrianImpact(px: number, pz: number, includePlayer = false, includePolice = true): PedImpact | null {
    ...
    for (let i = 0; i < this.cars.length; i++) {
      ...
      if (dist >= c.profile.radius + PED_REACH_MARGIN) continue;
  ...
  private drivePolice(
    ...
    const feeler = car.profile.radius + POLICE_FEELER_BASE + sp * 0.25;
    const fx = car.x + hx * feeler;
    const fz = car.z + hz * feeler;
    const probe = city.grid.resolve(fx, fz, car.profile.radius);
    const nx = probe.x - fx;
    const nz = probe.z - fz;
  ...
  private collide(city: City): void {
    for (let i = 0; i < this.cars.length; i++) {
      ...
      const fixed = city.grid.resolve(car.x, car.z, car.profile.radius);
    ...
    for (let i = 0; i < this.cars.length; i++) {
      for (let j = i + 1; j < this.cars.length; j++) {
        ...
        const o = circleOverlap(a.x, a.z, b.x, b.z, a.profile.radius + b.profile.radius);
        if (!o) continue;
  ...
  resolveActor(x: number, z: number, radius: number): { x: number; z: number } {
    ...
    for (const c of this.cars) {
      if (!c.active) continue;
      const o = circleOverlap(rx, rz, c.x, c.z, radius + c.profile.radius);
      if (o) {
        rx += o.nx * o.depth;
        rz += o.nz * o.depth;
      }
