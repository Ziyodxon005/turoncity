 ...
 *
 * Uses the legacy capsule+sphere mesh always (no FBX).
 */
export class Pedestrians {
  ...
  private stepPeds(w: World, dt: number): void {
    ...
    for (const e of w.query(Pedestrian)) {
      ...
      // fast threat bearing down on a path to hit them.
      ped.scared = false;
      if (threat) {
        ...
        if (dist < NEAR_RADIUS) {
          ...
          if (ts > VECTOR_SPEED && dist < PATH_LOOK) {
            ...
            const ahead = dx * tnx + dz * tnz;
            const lateral = dx * tnz - dz * tnx;
      ...
      if (!ped.scared && (fixed.x !== ped.x || fixed.z !== ped.z)) ped.heading += Math.PI;
  ...
  punch(x: number, z: number, dirX: number, dirZ: number): boolean {
    ...
    for (let i = 0; i < this.peds.length; i++) {
      ...
      if ((dx / d) * dirX + (dz / d) * dirZ < 0) continue;
      bestD2 = d2;
      best = i;
    }
  ...
  private drawPeds(w: World, alpha: number): void {
    ...
    for (const e of w.query(Pedestrian)) {
      ...
      if (ped.scared) {
        ...
        roll = Math.sin(this.tick * 1.7 + ped.x) * 0.4;
      }

      const ix = lerp(ped.px, ped.x, alpha) + sx;
      const iy = lerp(ped.py, ped.y, alpha);
      const iz = lerp(ped.pz, ped.z, alpha) + sz;
      const iHeading = angleLerp(ped.ph, ped.heading, alpha);
      const iTumble = lerp(ped.ptumble, ped.tumble, alpha);

      ped.group.position.set(ix, iy, iz);
      ped.group.rotation.set(iTumble, iHeading, roll);
    }
  }
}
