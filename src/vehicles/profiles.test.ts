...
describe('car profiles', () => {
  it('every profile is well-formed and uses a real body shape', () => {
    ...
    for (const p of [...PROFILES, INTERCEPTOR]) {
      ...
      ids.add(p.id);
      expect(p.radius).toBeGreaterThan(0); // every profile has a collision radius
    }
  ...
  it('collision radius scales with body size: truck > sedan > compact', () => {
    const truck = byId('bunker-hauler').radius;
    const van = byId('delivr-boxer').radius;
    const sedan = byId('crown-vantage').radius;
    const sports = byId('velocci-strada').radius;
    const compact = byId('komuter-bean').radius;
    expect(truck).toBeGreaterThan(van);
    expect(van).toBeGreaterThan(sedan);
    expect(sedan).toBeGreaterThan(sports);
    expect(sports).toBeGreaterThan(compact);
  });
});
