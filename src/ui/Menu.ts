...
export const DEFAULT_MODES: readonly GameModeOption[] = [{ id: 'explore', label: 'Erkin Sayohat' }];
...
export class Menu {
  ...
  constructor(
    ...
    titleBtns.append(
      this.button('O\'YNASH', () => this.cb.onPlay(), 'menu-play', true),
      this.button('YANGI O\'YIN', () => this.cb.onNewGame(this.chosenSeed()), 'menu-newgame', false),
    );
    ...
    this.pauseActions.append(
      this.button('DAVOM ETISH', () => this.cb.onResume(), 'menu-resume', true),
      this.button('QAYTA BOSHLASH', () => this.cb.onRestart(), 'menu-restart', false),
    );
  ...
  private optionsSection(): HTMLElement {
    ...
    wrap.appendChild(
      this.sliderRow('Ovoz', 0, 1, 0.05, this.opts.masterVolume, 'menu-volume', (v) => {
        this.opts = { ...this.opts, masterVolume: v };
        this.cb.onOptionsChange(this.opts);
      }),
    );
    wrap.appendChild(
      this.segmentedRow('Sifat', QUALITIES as readonly string[], this.opts.quality, 'menu-quality', (q) => {
        this.opts = { ...this.opts, quality: q as GameOptions['quality'] };
        this.cb.onOptionsChange(this.opts);
      }),
    );
    wrap.appendChild(
      this.sliderRow('Kun uzunligi', 30, 1800, 30, this.opts.dayLength, 'menu-daylength', (v) => {
  ...
  private controlsSection(): HTMLElement {
    ...
    h.textContent = 'BOSHQARISH';
    h.style.opacity = '.9';
    wrap.append(
      h,
      line('Haydash — WASD / strelkalar · <b>Bo\'sh joy</b> qo\'l tormozi · <b>F</b> kirish/chiqish · <b>[ ]</b> radio'),
      line('Piyoda — WASD · <b>Shift</b> yugurish · <b>Bo\'sh joy</b> musht · <b>F</b> mashinaga kirish'),
      line('Gamepad — RT/LT gaz · stick rul · <b>A</b> kirish/yugurish · <b>B</b> qo\'l tormozi · <b>X</b> musht'),
      line('<b>Esc</b> / Start — to\'xtatish · <b>R</b> qayta tiklash'),
    );
    return wrap;
  }
  ...
  private applyVariant(): void {
    ...
    this.header.textContent = title ? 'TURON CITY' : 'EHTIYOT PAUZA';
